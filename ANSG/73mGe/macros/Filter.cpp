...
Bool_t FilterFile(TString filename) {
  ...
  // Output variables for crystal trees
  Float_t outEnergy = 0;
  ...
  TTree *filteredTrees[N_CRYSTALS];
  TTree *unfilteredTrees[N_CRYSTALS];
  for (Int_t c = 0; c < N_CRYSTALS; c++) {
    ...
    unfilteredTrees[c] = new TTree(Form("crystal%d_unfiltered_tree", c),
                                   Form("Unfiltered events for crystal %d", c));
    unfilteredTrees[c]->Branch("energykeV", &outEnergy, "energykeV/F");
    unfilteredTrees[c]->Branch("xmm", &outX, "xmm/F");
    unfilteredTrees[c]->Branch("ymm", &outY, "ymm/F");
    unfilteredTrees[c]->Branch("zmm", &outZ, "zmm/F");
    unfilteredTrees[c]->Branch("eventTime", &outEventTime, "eventTime/i");
    unfilteredTrees[c]->Branch("liveTime", &outLiveTime, "liveTime/I");
    unfilteredTrees[c]->Branch("nInteractions", &outNInteractions, "nInteractions/I");
    unfilteredTrees[c]->Branch("interaction", &outInteraction, "interaction/I");
  }
...

  {
    ...
    for (Int_t c = 0; c < N_CRYSTALS; c++) {
      std::cout << filename << " crystal " << c << ": "
                << filteredTrees[c]->GetEntries() << " filtered / "
                << unfilteredTrees[c]->GetEntries() << " unfiltered"
                << std::endl;
    }
  }
  ...
  for (Int_t c = 0; c < N_CRYSTALS; c++) {
    ...
    unfilteredTrees[c]->Write("", TObject::kOverwrite);
  }
