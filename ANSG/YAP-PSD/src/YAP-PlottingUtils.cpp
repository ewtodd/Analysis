...
void PlottingUtils::PlotIndividualSpectra(
  ...
  for (const auto &pair : spectra) {
    ...
    std::string png_name = output_prefix + clean_name + ".png";
...
void PlottingUtils::PlotCalibrationCurve(TGraphErrors *calibration_curve,
  ...
  std::string png_name = output_name + ".png";
  c->SaveAs(pdf_name.c_str());
  c->SaveAs(png_name.c_str());
  delete c;
}
...
void PlottingUtils::PlotSinglePeakFit(const CalibrationPeak &peak,
  ...
  if (peak.fit_successful) {
    ...
    TF1 *bkg_only = new TF1("bkg_only", "[0] + [1]*x", peak.fit_range_low,
                            peak.fit_range_high);
    ...
    sprintf(buf, "Bkg: %.1f + %.2e*x ", peak.fitted_bkg_const,
            peak.fitted_bkg_slope);
    text->DrawLatex(x_pos, y_pos, buf);
  ...
  size_t pos = png_filename.find(".pdf");
  if (pos != std::string::npos) {
    png_filename.replace(pos, 4, ".png");
...
void PlottingUtils::PlotAverageWaveforms(const std::vector<Float_t> &f_alpha,
  ...
  leg->AddEntry(gr_gamma, "Co-60 (#gamma)", "lp");
  leg->Draw();
  ...
  c->SaveAs((output_name + "_log.png").c_str());
...
void PlottingUtils::PlotSIWeightingFactor(
  ...
  std::string png_name = output_name + "_with_weighting.png";
  c->SaveAs(pdf_name.c_str());
  c->SaveAs(png_name.c_str());

  std::cout << "SI weighting factor plot saved: " << pdf_name << " and "
            << png_name << std::endl;
...
void PlottingUtils::PlotPSDSpectra(
  ...
  for (const auto &pair : psd_spectra) {
    ...
    std::string png_name = output_prefix + "_" + clean_name + ".png";
    c->SaveAs(pdf_name.c_str());
    c->SaveAs(png_name.c_str());
...
void PlottingUtils::PlotPSD2D(const std::string &waveforms_file,
  ...
  for (auto &pair : hist2d_map) {
    ...
    std::string png_name = output_name + "_2d_" + clean_name + ".png";
    c->SaveAs(pdf_name.c_str());
    c->SaveAs(png_name.c_str());
...
void PlottingUtils::PlotPSDWithFOM(TH1F *hist_alpha, TH1F *hist_gamma,
  ...
  leg->AddEntry(hist_gamma, "Co-60 (#gamma)", "f");
  ...
  std::cout << "PSD+FOM plot saved: " << pdf_name << " and " << png_name
            << std::endl;
...
void PlottingUtils::PlotAverageWaveformsByLightOutput(
  ...
  // Source information
  std::map<Int_t, std::string> source_names = {{0, "Am-241"}, {3, "Co-60"}};
  std::vector<Int_t> sources_to_plot = {0, 3};
  ...
  for (Int_t src_id : sources_to_plot) {
    ...
    std::cout << "Saved: " << pdf_name << " and " << png_name << std::endl;
