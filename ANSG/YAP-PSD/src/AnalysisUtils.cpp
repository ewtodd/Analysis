...
std::vector<Float_t> AnalysisUtils::CalculateSIWeightingFactor(
  ...
  for (size_t i = 0; i < min_length; ++i) {
    ...
    if (sum > 1e-2) {
      if (i > 25) { // Avoid division by zero
        p_t[i] = (alpha_norm[i] - gamma_norm[i]) / sum;
      } else if (alpha_norm[i] > 0.1 && gamma_norm[i] > 0.1) {
        p_t[i] = (alpha_norm[i] - gamma_norm[i]) / sum;
      }
    } else {
      p_t[i] = 0.0;
...
Bool_t AnalysisUtils::CalculateChargeComparisonPSD(
  ...
  if (!histograms_already_filled || force_recalculate) {
    ...
    std::map<Int_t, std::string> source_names = {
        {0, "Am-241"},        {1, "Cs-137"},          {2, "Na-22"},
        {3, "Co-60"},         {4, "Am-241 & Cs-137"}, {5, "Am-241 & Na-22"},
        {6, "Am-241 & Co-60"}};
...
TH1F *AnalysisUtils::CalculateChargeComparisonPSDOnTheFly(
  ...
  // start and end entries computed from number of accepted for each source from
  // initialprocessing (see initial.stats file)
  if (source_id_calc == 0) {
    start_entry = 0;
    end_entry = 1084145;
  } else if (source_id_calc == 3) {
    start_entry = 1084145 + 8102466 + 1319632;
    end_entry = 1084145 + 8102466 + 1319632 + 6673660;
  } else {
